...
对于单组询问，首先计算 $f_{d}$ 表示 $d\vert \gcd$ 的答案，对 $f$ 做一遍狄利克雷差分（莫反）就是 $d=\gcd$ 的答案。枚举 $d$ 之后，问题变成选 $n$ 个 $\lfloor\frac m d\rfloor$ 以下的数，要求 $\text{lcm}>\lceil\frac A d\rceil$ 。上取整变成全体减下取整方便一点。然后再枚举 $\text{lcm}\vert v$，则满足：
